...
class Shopping_Ecommerce_WP_theme_option{
...
function menu_tab() {
    $menu_title = esc_html__('Shopping Ecommerce Wp Options', 'shopping-ecommerce-wp');
    add_theme_page( esc_html__( 'Shopping Ecommerce Wp', 'shopping-ecommerce-wp' ), $menu_title, 'edit_theme_options', 'texture_started',array($this,'tab_page'));
...
wp_enqueue_style( 'texture-started-css', get_template_directory_uri() . '/lib/texture-option/assets/css/started.css' );
wp_enqueue_script( 'shopping-ecommerce-wp-admin-load', get_template_directory_uri() . '/lib/texture-option/assets/js/texture-options.js', array('jquery', 'updates' ), true );
...
$data = apply_filters(
    ...
    wp_localize_script( 'shopping-ecommerce-wp-admin-load', 'THAdmin', $data); 


} else {

    wp_enqueue_style( 'texture-notify-css', get_template_directory_uri() . '/lib/texture-option/assets/css/notify.css' );  
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('wptexture','shopping-ecommerce-wp'),
    'theme_brand_url' => esc_url($themedata->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'shopping-ecommerce-wp'), esc_html($themedata->get( 'Name' ),'shopping-ecommerce-wp'), $themedata->get( 'Version' ) ),
    'welcome_desc' => esc_html($themedata->get( 'Name' ).' is an eCommerce WordPress theme specially made for shopping websites.', 'shopping-ecommerce-wp' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','shopping-ecommerce-wp'),
       'post_content' => '',
       'post_status' => 'publish',
     ...
     /*
      ...
      if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
        wp_send_json_error(
          array(
            'success' => false,
            'message' => __( 'No plugin specified', 'shopping-ecommerce-wp' ),
          )
        );
      }
      ...
      wp_send_json_success(
        array(
          'success' => true,
          'message' => __( 'Plugin Successfully Activated', 'shopping-ecommerce-wp' ),
        )
      );
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'shopping-ecommerce-wp' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'shopping-ecommerce-wp' );
                   $detail_link = $install_url = '';
                }

            if ( ! is_plugin_active( $plugin_init ) ){
                    $button_txt = esc_html__( 'Install Now', 'shopping-ecommerce-wp' );
                    if ( ! $status ) {
                        ...
                        $button_txt = esc_html__( 'Activate Now', 'shopping-ecommerce-wp' );
                    }
                }
...
$boj = new Shopping_Ecommerce_WP_theme_option(); ?>